...
**残差が出たら、サイクルを重ねる前に `docs/notes/cw132-hardblock-index.md` を症状で引く**: A 群 (即 park 可) 一致なら追加サイクルを焼かず退避 (HANDOFF に pattern 名 + 出典)、B/D 群なら recipe を当てる、C 群なら cheap probe 1 回。詳細は `docs/per_fn_matching_strategy.md` §4。
...
compiler 内部を frida で実行時 trace するのは **main 主導の研究活動** (`docs/notes/cw132-compiler-tracing-playbook.md`)。subagent は frida を起動しない (重く、priv copy + Ghidra 常駐が要る)。残差判定は索引で済ませる。

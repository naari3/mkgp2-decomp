...
- `prompts/cycle.md` — CronCreate (3min 間隔) で発火する main の 1 cycle prompt
...
CASE 1 (最優先): drain.flag が立っている
  drain mode の処理:
    - active_subs が 0 → CronDelete (cycle cron 停止)、session_end ログ、return
    - active_subs ≠ 0 → 通知を待つだけ。新規 dispatch しない
    - 通知 hook で merge は通常通り走る

CASE 2 (旧 CASE 3): 失敗 batch の再編成
...
CASE 4 (旧 CASE 5): pending batch 無く pending function あり → main が新規 batch 編成
  編成処理 (extab-aware):
    ...
    - 同 cycle 内で CASE 3 へ fall through して即 dispatch (chain)
  大規模 group (>10 fn) は別レーン: HANDOFF_TO_USER.md に escalate して放置
  (詳細ルールは prompts/cycle.md CASE 4、extab map は tools/build_extab_map.py 参照)

CASE 5 (旧 CASE 6, 該当なし): "no action" を 1 行 print して return
  次 fire は cron (3min 後) が担当。ScheduleWakeup は cron と二重起動になるので使わない
  (sub 完了通知が来れば cron fire を待たず即 system-reminder で届いて処理される)

完了判定:
  pending=0 かつ in_progress=0 かつ active_subs=0 かつ全 batch terminal
  → HANDOFF_TO_USER.md に "全 batch 完了" 書き、CronDelete で cycle cron 停止
  これ以外で cycle cron を停止してはならない
```

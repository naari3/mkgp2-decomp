...
| `CarObject_HandleItemEffect` (auto_ONKARTHIT 2026-06-10) | **dead-counter-in-ctr-loop** (新 precan class): target の ctr loop が読まれない up-counter web (`li r4,0` + `addi r4,r4,1`) を保持、CW 1.3.2 は 8 probe 全て (inline asm + volatile_asm 含む) でこの web を DCE。98.62% (494 命令中 89 diff 行、うち 84 は inline site 2/3 の register 順列 = counter web 欠落の cascade と推定) | -O4,p の iterative DCE は never-read web を必ず消す。`#pragma opt_dead_assignments off` も効かず。do-while にすると ctr 変換が消える (二律背反) | **precan 式: mtctr+bdnz loop 内に「次の write まで read されない addi rX,rX,1」があればこの class** (0-2 probe で見切り)。98.6% C + decode 済み構造 + lane-clear inline helper recipe を `docs/drafts/CarObject_HandleItemEffect_98pct.handoff.md` に保存。詳細 `docs/notes/cw132-dead-counter-ctr-loop-hardblock.md` (`-inline auto,deferred` が extab_user TU を壊す負例も記載) |
| smallrun batch 4 fn (auto_ONKARTHIT 2026-06-10) | (1) wrapper の **prologue load hoist**: target が `lwz rX,off(r3)` を LR-save `stw r0,0x14(r1)` より上に置く (AdvanceAnim3c 77.8% / GetCurrentSpeedWithBonus 80.0%、後者は左→右 arg emission も)。(2) **return-pinned f1 への dying divisor coalesce** (CalcSpeedRatio 97.65%)。(3) **SR init の mr-vs-li const-fold** (GetMaxSpeedWithBonus 94.35%、残 2 命令)。(4) **lbz の load-over-store + r3 coalesce 両立不能** (GetBoostArmedAndTimer) | CW 1.3.2 は load を store より上に scheduling しない (alias 非分析) + return coalescing は named local で消える。probe battery は note 参照 | `docs/notes/cw132-prologue-load-hoist-unreproduced.md` に詳細 + **precan rule** (wrapper で hoisted load + compare 無し → 1 probe で見切る) + 勝ち idiom 4 種 (condition-assign entry / compound f1 chain / `(float)__fabs` / explicit off induction) |

...
## 2026-06-13: step 9 — StlList_InsertBefore byte-exact push (option a) → 構造要素は個別に一致、合成が壁

`tmp/dtorcpp/stllist{2..6}.cpp` で StlList_InsertBefore を byte-exact に詰める試み。判明した要素:
- **hidden-pointer iterator 返し**: `Iter` に copy ctor を持たせると CW が struct を memory 返し (hidden ptr r3)
  → 4 引数 (out/this/pos/value) が callee-saved になり **`stmw r27`/`lmw` 一致** (`stllist4.cpp`)。
  trivial Iter (copy ctor なし) は register 返し → 3 引数 → `stmw` 不一致。
- **alloc-null 検査なし**: target は `if(n)` を持たず placement-new の `addic. node+8; beq` guard のみ。
- **EH cleanup tail** (`b L; bl TimedFree; L:`、dead code、extab 8B=action なし): try/catch (`stllist2`) で出るが、
  copy-ctor + try/catch (`stllist4`) だと CW が construct=nothrow と判定し cleanup を**削除**。
  NodeGuard idiom (`stllist5/6`) は cleanup tail を保つが register 返しに戻り bloat (0x100/0x108)。
- **未合成**: hidden-ptr 返し (copy ctor) と cleanup 保持 (guard/try) が source 上で噛み合わない。
  両立には MSL std::list::insert template の正確な構造 (allocator construct + node guard + iterator class) の
  coherent な再現が要る。MSL header 無しの trial では idiom が互いに干渉し 6 試行で未収束。

**現実的評価**: stllist4 が最接近 (hidden-ptr 返し/stmw/splice/placement-new guard/8B extab 一致、size 0x90 vs 0xA8)。
残差 = target が持つ **dead な cleanup code** (8B extab なので EH 実行はされない = 機能等価)。
StlList の完全 byte-exact は MSL list::insert template の coherent 合成が必要で、深い idiom-matching。
標準 dtor 9 fn は byte-exact 済。hard 3 fn (KartItem value-struct / 0xB0 reg-alloc / StlList template) は
各々この種の「原ソース idiom 依存の last-mile」で、構造は再現済・残差は機能無関係な CW/MSL artifact。

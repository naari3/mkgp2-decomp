...
## clFlowItem Draw/Update: closed-class の再出現 2 例 (2026-07-19, unit-first claim #32)

台帳追記 (どちらも既知 class の再確認。詳細な probe 経緯は
`docs/per_fn_matching_strategy.md` §19.5、C body は src/game/clFlowItem.c の #if 0):

- **clFlowItem_Draw** (asm_fn park, C 96.6% / 命令列完全一致):
  strength-reduction walker web が死にゆく base pointer (icons/grid) の register に
  合流せず `mr` 2 本過多。= move-coalescer class (HandleItemEffect handled-web と同型)。
  OBSERVED negative: decl order 34 perm / member fn 化 (this は r31 に乗るが walker
  不動) / register hint 全て不発。**新観測**: destructive-walk 書き換え
  (`p = (T *)((char *)p + 4)`) は命令数を合わせるが web が 1 本増えて
  callee-saved が 6→7 本、frame +0x10 になる — この class の source lever としては
  逆効果。
- **clFlowItem_Update** (asm_fn park, C 99.88% / 残差 6 insn):
  1 変数 (sel/pd 統合、cast 別名) にしても代入点で live-range split が起き、
  split range が独立に色付けされて r31/r24 に分かれる (target は両 range とも r31)。
  variable-merge lever の再燃焼 = CarObject_Init ch=o1 で REFUTED 済みの形。
  decl order 36 perm / scoped local 化も不発 (±0)。

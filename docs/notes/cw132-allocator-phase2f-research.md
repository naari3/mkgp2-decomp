...
## VN 代表選択の実行時 trace = 「fold は copy-backed 代表のみ」を実測 (2026-06-12, /loop iter 10, user:「追うべきだ」)

ユーザー指示「frida で VN の代表選択を実行時 trace し『なぜ我々の場合 bool web が value-0 の代表に
ならないか』を読む」を実行。前節 CONCLUSION の「定数-0 オペランドが bool web と同 value-number を持つ」
という想定を実測で**精緻化/一部訂正**した。

### 機構の追加 white-box (decompile, fact)
- `FUN_005093f0(node, src)` = 値番号の割当 (undo-journal 付き)。src==0 → 新規 value-id
  (DAT_005e8abc++)、src≠0 → src の web の value-id を継承し node+0x20 に rep-source-**node** を記録。
  **VN-table[web][1] (代表 web) は触らない**。
- `FUN_005092b0(srcRec, dstRec)` = copy 伝播。`dst = src` のとき dst.value-id = src.value-id、
  かつ **dst の VN-table 代表 web (piVar8[1]) = src.rep (無ければ src の web 自身)** を設定。
  → **VN-table の代表 web を非ゼロにするのは FUN_005092b0 (と FUN_00509010 経由の同呼び出し) だけ**。
- `FUN_005086a0` (immediate/leaf def, flag&8&0x1000100) と arithmetic def (flag&4) は def-web の
  代表を**クリア** (piVar8[1]=0) し新 value-id を振る。fold 置換 (FUN_00508140@0x5081c8) が読むのは
  piVar8[1] なので、**copy で代表が付いた web しか fold 置換の対象にならない**。

### 実行時 trace (probe_foldcond.js / run_probe.py, probe_ef.c コンパイル, OBSERVED=fact)
hook 点: 0x508182 (fold-eligibility を通過した use-operand 本体。branch-target でなく安全) で native
と同条件を JS 複製し全数記録 + FUN_005092b0@0x5092b0 entry で copy 伝播を記録。
- eligible use-operands=1267 (class 別 {0:14, 1:103, 3:188, 4:962})、**fold 発火=18 (全 class-4)**、
  copy 伝播=67。→ fold 機構は我々のコードでも普通に働く (positive control 成立)。
- **18 件の fold は全て非ゼロ値**: val=494/386/488/387/384/496/381。pattern は (web N, web N+1) → rep N-3。
  例 web75,76→rep72 / web88,89→rep85 / …。これは **64bit field-load (itemBus->flags) 繰り返し読みの
  CSE** (TARGET も当然やる共通処理であり、param flip の差分ではない)。
- **18 件全てに対応する COPY が存在** (web75⇔`COPY dst75⇐src72,val494`, web88⇔`COPY dst88⇐src85`, …全件)。
  → 「fold は copy-backed 代表のみ」を実測で裏付け。
- **bool web (32/33 → r25/r26) は計算値の copy-DEST**: `COPY dst(32)⇐src(3,val=127)`,
  `COPY dst(33)⇐src(4,val=128)`。値は 127/128 (= bool の計算結果) で **value("0") ではない**。
  bool web を SOURCE とする value-0 の copy は皆無 → **bool web は value-0 の代表に決してならない**。
- 定数-0 (AND 上位語) は低位 scratch web(3/4 = volatile r3/r4) に毎回 `li` で materialize され
  (FUN_005086a0 経路 = 代表クリア)、bool web と統合されない。

### 訂正/精緻化された CONCLUSION
fold の発火条件は前節想定の「定数-0 と bool が同 value-number」**ではなく**、より厳しく
**「AND の 0-オペランドが COPY 命令の dst として生成され、その src(=代表 web) が当該 bool reg である」**
こと。代表 web は copy 伝播 (FUN_005092b0) だけが付与し、immediate `li 0` 経路は代表を付けない。
- TARGET の `and r0, flags_hi, rBool` = IR 上で AND の 0-源が **bool(=0 を持つ callee-saved reg) からの
  copy** として生成され、fold がその copy dst を bool web に巻き戻した結果。
- 我々の IR では AND の 0-源は常に fresh immediate (`li r4,0`) で、copy ではないため代表を持たず fold 不可。
- 「bool の 0 を AND の 0-源として再利用する (copy 化) か、fresh li するか」は **ValueNumbering より上流の
  命令選択 (InstrSelection) の判断**。前段の経験的掃き出し (最小ケース全滅 / 7000中2関数 / C・C++不問 /
  高レジスタ圧でも不発) は、この命令選択判断が source の型・構造・レジスタ圧から制御できないことを示す。

→ VN レイヤの追跡はこれで底を打った (代表選択機構を完全 white-box + 実測)。残る唯一の未踏層は
**InstrSelection が「既存 0-reg 再利用 (copy) vs fresh materialize」を決める箇所**。ただし経験則は
source 非制御を強く支持しており、ここを開いても誘発レシピが出る確度は低い (= park 維持の結論は不変)。
assets: tmp_probe/probe_foldcond.js, tmp_probe/run_probe.py。

## 実 asm で grounding 再確認: 16 diff の正体と summary register 記述の訂正 (2026-06-12, /loop iter 11)

summary 由来の記述に頼らず、TARGET 実 asm (build/GNLJ82/asm/game/auto_ONKARTHIT_block.s) と我々の最良
再構成 probe_ef.c の objdiff (verify_any.py, reg+fp マスク) を現物で突き合わせた。

OBSERVED (16 genuine diffs, reg/fp 正規化後, fact):
- **FP scheduling 4 件** (idx 81/82, 89/90): `fsubs` のオペランド順・命令位置違い。fold とは独立の別問題。
- **bool-dispatch 12 件** (idx 221–268, 4 bool 各 3 件): 下記 fold 1 点に全帰着。

実 asm 対比 (b28 block, mask 0x20000000):
- TARGET 8004A5A0: `li r28,0` (bool b28=0, callee-saved) **1 個**を `and r0,r0,r28` / `xor r3,r3,r28` /
  `xor r0,r0,r28` で **3 回再利用**。分岐 `bne L_set; b L_done`。bool は r28/r27/r26/r25 に常駐し各々
  の 0-init をその場の 64bit比較の 0 に兼用。
- OURS 00000368: `li r4,0` (transient 0) と `li r30,0` (bool, callee-saved) の **0 が 2 個**。and/xor は
  r4 を使い、bool は r30。分岐 `beq L_done`。
- → 差は「bool の 0 と 64bit比較が要求する定数 0 を 1 web に統合 (fold) するか、寿命差 (bool=永続 callee /
  比較0=transient) で別レジスタに分割するか」。fold すれば `li 0` が 1 個に減り分岐も bne;b になる。

訂正: 前々節までの「TARGET は callee-saved bool reg を AND の 0 源に再利用」は **この 4-bool block では
正しい** (r28 を再利用)。ただし途中で私が早期ガード (8004A270, mask 0x80, volatile r4 使用, bool 常駐
なし) を 4-bool block と取り違えた経緯がある → 早期ガードは fold 対象外 (bool を保持しないため)。fold は
「bool を callee-saved に常駐させる block」でのみ問題化する。

NEGATIVE (新規否定実験, fact): probe_ef_neb.c = 4 つの比較を `(flags & MASK) != 0` から
`!= b28` (b28==0 の bool 変数自身) に変更 → 出力は baseline と **完全一致 (16 diff 同一)**。mwcc は
b28==0 を承知で `!= 0` に正規化し、比較 RHS を bool 変数にしても fold は誘発されない。
→ 「比較を bool に対して書けば bool が value-0 代表になる」仮説は否定。

CONCLUSION (不変): 16 diff = fold (12) + FP schedule (4) の 2 独立障害。fold は VN 定数統合で source
非制御 (本ノート前節 + 全否定実験)、FP schedule は別の scheduling artifact。**両方を同時に source で
解消する手段は無く、OnKartHit の byte-identical clean-C 化は現状不可 → asm_fn park 維持が妥当**。
assets 追加: tools/compiler_probe/onkarthit_vn/probe_ef_neb.c。

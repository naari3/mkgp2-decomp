...
| pattern | 症状 | 状態 | 出典 |
|---|---|---|---|
| **prologue-load-hoist** | wrapper/thunk/forwarder の prologue で member load (`lwz r3,off(r3)`) が LR-save (`stw r0,0x14(r1)`) より上にあるのに、CW は LR-save を先に出す。源: `return f(self->field, …)` 形の forwarder 全般 | compare-consuming-the-load がある wrapper のみ CW が latency-driven hoist する (matched)。compare 無の純 forwarder は不可。**2026-06-12 OnKartHit TU の GetCurrentSpeed/AdvanceAnim 等で 35+ source 形 + 全 pragma/opt-level を sweep → 全滅再確認**。cheap probe 1 回後 skip | [cw132-prologue-load-hoist-unreproduced.md](cw132-prologue-load-hoist-unreproduced.md) |
| **leaf store/return-read tie-break** | leaf getter で「副作用 store (`stfs/stw → *out`)」と「戻り値 read (`lbz/lwz → r3`)」が独立し順序自由なとき、CW は store 先、target は read 先 (or 逆)。例: `*out=self->f; return self->b;` → CW `lfs;stfs;lbz r3` だが target `lfs;lbz r3;stfs` | **解決 (2026-06-12)**。〜2026-06-11 は「source 非制御・即 park」と結論していたが**訂正**: scheduler 白箱化で store-vs-load は tie-break ではなく保守的 aliasing **依存エッジ** (向き=source順) と判明。target `lfs;lbz r3;stfs` は (1) load-first source で store を最後に回し、(2) float read を `*(volatile float*)&...` で **pin** し lfs を byte-load より先に emit すれば再現。GetBoostArmedAndTimer を byte-identical promote 済 (`main.dol: OK`)。旧 sweep は volatile **store** を試したが volatile **read** (load pin) を欠いていた | [cw132-scheduler-store-load.md](cw132-scheduler-store-load.md) |
...
| **fdivs-divisor-coalesce-into-pinned-f1** | leaf で `ratio = speed / d; clamp; return ratio` の fdivs が、target では「死ぬ divisor を return-pinned f1 web に寄せる」(`fdivs f1,f2,f1`)。C 再現は **構造 100% 一致**だが register 割当だけ違う | **解決 (2026-06-12)**。FP colorer 白箱化 (FUN_00507a30 = lowest-available、FP class=3) で機構同定: divisor を別 web にすると result(pinned f1)と interfere → f0。**レバー = in-place 除算** (`r = mv->...refSpeed; r = speed / r;`) で divisor を return 変数に load し被演算子 web と result web を単一化 → `fdivs f1,f2,f1`。CalcSpeedRatio を byte-identical promote 済 (`main.dol: OK`) | [cw132-fdivs-coalesce-pinned-f1.md](cw132-fdivs-coalesce-pinned-f1.md) |

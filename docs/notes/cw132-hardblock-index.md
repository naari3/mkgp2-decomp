...
| **leaf store/return-read tie-break** | leaf getter で「副作用 store (`stfs/stw → *out`)」と「戻り値 read (`lbz/lwz → r3`)」が独立し順序自由なとき、CW は store 先、target は read 先 (or 逆)。例: `*out=self->f; return self->b;` → CW `lfs;stfs;lbz r3` だが target `lfs;lbz r3;stfs` | source 非制御 (実証)。**4 命令の GetBoostArmedAndTimer を exhaustive sweep: scheduling off/optimization_level 1-3/volatile store/comma 全て store 先で固定**。named-local 戻り値は `r0 + mr r3,r0` の余剰命令。即 park | [cw132-hardblock-index.md](cw132-hardblock-index.md) (本エントリ) |

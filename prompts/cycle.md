...
- **1 cycle = 1 heavy action、light は chain 可**:
  - **heavy** (1 cycle 1 件まで): CASE 2 merge (parse + apply + build verify 1-2 分 + commit)
  - **light** (chain 可): CASE 5 编成 → CASE 4 dispatch、CASE 3 再编成 → CASE 4 dispatch、CASE 4 を active_subs 上限まで連続 dispatch
  - chain しても context 圧迫が浅い (state.json 編集 + Agent invocation のみ)
  - merge と編成 を混ぜない (heavy + light の同 cycle 詰め込みは context が重くなる)
- **conflict 解決は main の手で**: merge tool が conflict 検出したら abort、main が Edit で resolve (rollback 自動化は tool に入れない)

## 1 cycle 動作

**1 cycle = 1 heavy action 原則**: 以下の CASE のうち最も heavy なのは CASE 2 merge (1/cycle 上限)。light な action (CASE 3/4/5) は chain して 1 cycle 内で複数実行してよい。ただし heavy + light の混在は避ける (merge した cycle で続けて編成・dispatch しない)。
...
CASE 5 (编成) → CASE 4 (dispatch) は **同 cycle 内 chain 可** (両方 light、
state.json 編集 + Agent invocation のみで context 圧迫は浅い)。
CASE 3 (失敗 batch 再编成) → CASE 4 (dispatch) も chain 可。
CASE 4 自体も active_subs < 3 の上限まで連続 dispatch して構わない。
heavy な CASE 2 (merge) と light な CASE 3/4/5 の混在は避ける。

**parallel 化の原則**: dispatch cycle (heavy 無し) では **active_subs を上限まで埋める**
ことを目標にする。各 cycle で:
1. active_subs を確認
2. **`while len(active_subs) < 3 and (pending batch あり or pending fn あり)`**:
   - pending batch あれば CASE 4 dispatch
   - pending batch 無ければ CASE 5 編成 → 直後に CASE 4 dispatch (chain)
3. 上限到達 or 編成可能な fn 枯渇で return

「1 batch 編成して 1 dispatch して終了」は **明確な anti-pattern**。
parallelism を諦めて user に約 20 min の空白を与える結果になる。
唯一の例外: 直前に CASE 2 (merge) を実行した cycle (heavy + light 混在禁止)。
...
for batch_id, batch in state['batches'].items():
    ...
    atomic_write_state()
    return   # 1 cycle 1 heavy action (merge した cycle はここで return、続けて編成 / dispatch しない)
```
...
if not pending_fns:
    ...
    atomic_write_state()
    # chain: そのまま CASE 4 へ fall through (active_subs < 3 なら即 dispatch)
```
...
- 1 cycle 1 heavy action を厳守 (merge は 1/cycle)。light (编成 / dispatch) は chain 可、ただし merge した cycle に続けて編成・dispatch しない

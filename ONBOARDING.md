...
- byte-identical rebuild OK: `python configure.py && ninja` で
  `build/GNLJ82/main.dol: OK` (SHA-1 一致) する状態を常時維持
- 関数数: **7,608** (symbols.txt 上の function symbol)
- named symbols: **~3,035 / 7,608 (~40%)**
- Matching TU 経由の関数: **~470** (うち asm_fn 退避 ~76 を含む。
  正確な内訳は orchestrator state / progress page 参照) / NonMatching 8
- pending (named で未着手): **~2,557**
- **対象選定は unit-first** (2026-07-19 採用): `python tools/plan_units.py` で
  ランキングを出し、意味のある unit (struct/class クラスタ) を 1 つ選んで完食する。
  正本は `docs/unit_first_strategy.md`
- 数値の再計算: `python tools/plan_units.py` (SoT 直読み、state.json 不要)
...
| matching workflow (手順、命名規約) | repo 内 `.claude/skills/mkgp2-match/SKILL.md` (2026-07-18 に user scope から移設、user scope 側は pointer stub) |
| Ghidra MCP 接続など naari3 環境固有の手順 | `~/.claude/skills/` (user scope skill: mkgp2-ghidra / ghidra-mcp) |

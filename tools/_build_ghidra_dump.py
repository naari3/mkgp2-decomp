#!/usr/bin/env python3
"""Build ghidra_symbol_dump.json from raw Ghidra dump files.

Inputs (both regenerated by tools/ghidra_dump_raw.java via run_script_inline):
  - tools/_ghidra_dump_functions.json: {"functions": [{"name", "address",
    "isThunk", "isExternal"}, ...]}
  - tools/_ghidra_dump_globals.txt: plain text "name @ addr [Type] (DataType)"
    lines (list_globals-compatible format)
...
FN_RAW = HERE / "_ghidra_dump_functions.json"
...
def load_functions() -> list[dict]:
    return json.loads(FN_RAW.read_text(encoding="utf-8"))["functions"]

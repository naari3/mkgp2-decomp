...
- `prompts/cycle.md` — CronCreate (3min 間隔) で発火する main の 1 cycle prompt 本体
...
`prompts/cycle.md` は main 自身が cron fire 毎に実行する手順書。それ以外は方針 / 規範。
...
| dispatch / 編成 / 再編成 / cleanup | cycle (cron fire) | active_subs < 6 上限まで chain |
...
「merge hook → active_subs 減 → そのまま turn 終了」は **anti-pattern**。pending work があるのに cron heartbeat (3 min) を待つ状態は cycle 設計上発生してはいけない。
...
5. **`extab_group=None` を信じて group text range 内の fn を dispatch する** (2026-06-10 実例: batch_text_80051cb8_itemtable_find / batch_text_80050010_itemeffect_dispatch): state.json の `extab_group` は「fn 自身が extab entry を持つか」で populate されるため、**extab entry を持たない fn は巨大 group の .text range 内に居ても None になる**。そういう fn の wedge TU は dtk が `Split ... overlaps with previous split` で拒否 (group 内部の bisect 不可。過去の §15 wedge が通ったのは group の外側 boundary だったから)。**正解**: CASE 4 編成 pre-check で batch member 全員の addr を `.orchestrator/extab_groups.json` の **group .text range と照合** する。range 内なら group member 扱い (>10 fn group なら escalate)。最大の地雷原は `auto_fn_8004A238_text` = .text 0x8004A238..0x80052F20 / 107 fn — この range 内の named pending (~40 fn) は mega-bundle scaffold 完了まで全て dispatch 不可
6. **sub が build を `run_in_background` で起動したまま return する** (2026-06-10 実例: jvscalib / iseslot_lifecycle の 2 sub 連続): sub の「completed」notification が来るが HANDOFF.md も commit も無い。**対策**: dispatch prompt に「ninja / configure.py は必ず foreground で実行し完了を待て (`run_in_background` 禁止)。HANDOFF.md を書いて commit するまで return しない」を明記する。**main 側 recovery**: worktree に成果 (src + config diff) が残っていれば main が build verify (`ok` target 生成済みなら SHA-1 通過の証拠) → 手動 merge。objdiff report (`ninja build/GNLJ82/report.json`) で per-fn match% を必ず確認 — SHA-1 fail なら部分 match の可能性 (lifecycle 例: 3 fn 中 2 fn は 100%、1 fn 80% で残walk)
